...
#define CAPS_LED_DUTY_CYCLE   222
#define CAPS_LED_FULL_CYCLE   333
...
layer_state_t layer_state_set_user(layer_state_t state) {
    ...
    if (is_caps_word_on()) {
        ...
        if(caps_lock_led_state){
            if(timer_elapsed(caps_lock_led_timer) >= (CAPS_LED_FULL_CYCLE - CAPS_LED_DUTY_CYCLE)){
                writePinLow(CAPS_LOCK_LED_PIN);
                caps_lock_led_state = false;
                caps_lock_led_timer = timer_read();
            }
        } else {
            if(timer_elapsed(caps_lock_led_timer) >= CAPS_LED_DUTY_CYCLE){
                writePinHigh(CAPS_LOCK_LED_PIN);
                caps_lock_led_state = true;
                caps_lock_led_timer = timer_read();
            }
        }
